...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, u, v, layers):
        ...
        X = np.concatenate([x, y, t], 1) #合并
        
        self.lb = X.min(0) #每列的最小值
        self.ub = X.max(0)
        ...
        # Initialize NN
        self.weights, self.biases = self.initialize_NN(layers) #偏置项
        ...
        self.sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, #允许 tensorflow 自动分配设备
        ...
        #优化器  https://docs.scipy.org/doc/scipy/reference/optimize.minimize-lbfgsb.html
        ...
        #global_variables_initializer返回一个用来初始化计算图中所有global variable的op。 不必深究
    ...
    def xavier_init(self, size):
        ...
        #截断的产生正态分布的随机数，即随机数与均值的差值若大于两倍的标准差，则重新生成 stddev，标准差
    ...
    def neural_net(self, X, weights, biases):
        ...
        for l in range(0,num_layers-2):
            W = weights[l]
            b = biases[l]
            H = tf.tanh(tf.add(tf.matmul(H, W), b))#matmul 矩阵乘法
        W = weights[-1]
        b = biases[-1]
...
def plot_solution(X_star, u_star, index):
    ...
    ub = X_star.max(0)
    nn = 200
    x = np.linspace(lb[0], ub[0], nn) #线段数据
    y = np.linspace(lb[1], ub[1], nn)
    X, Y = np.meshgrid(x,y) #连成x-y二维画布
...
def axisEqual3D(ax):
    # getattr(obj,name[,default]) 其中obj为对象名，name是对象中的属性，必须为字符串 获取对象中的属性值
...
#    plot_solution(X_star, p_star - p_pred, 5)
    ...
    divider = make_axes_locatable(ax)#定位色条
    ...
    #product(A,B)函数,返回A和B中的元素组成的笛卡尔积的元组 combinations实现排列组合
    for s, e in combinations(np.array(list(product(r1,r2,r3))), 2):
        if np.sum(np.abs(s-e)) == r1[1]-r1[0] or np.sum(np.abs(s-e)) == r2[1]-r2[0] or np.sum(np.abs(s-e)) == r3[1]-r3[0]:
            ax.plot3D(*zip(s,e), color="k", linewidth = 0.5)   
