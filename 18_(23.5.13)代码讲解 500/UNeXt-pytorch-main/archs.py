...
#         return x[:, :, pad:-pad, pad:-pad]
    ...
    def forward(self, x, H, W):
        ...
        x_shift_r = x_s.transpose(1,2) #转置操作
...
class shiftedBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        #Drop Path是一种正则化技术，用于避免模型的过拟合。使用nn.Identity()函数创建一个恒等映射层
...
class UNext(nn.Module):
    ...
    def forward(self, x):
        ...
        out,H,W = self.patch_embed3(out) #图像转为补丁
        ...
        _,_,H,W = out.shape
        #将一个三维的PyTorch张量out变为一个二维的矩阵，并将矩阵的行和列进行交换
        ...
        out = self.dnorm3(out)
        out = out.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        #F.interpolate是PyTorch中的一个函数，用于对张量进行插值操作。主要用途是对输入张量进行尺寸调整或图像缩放等操作。
