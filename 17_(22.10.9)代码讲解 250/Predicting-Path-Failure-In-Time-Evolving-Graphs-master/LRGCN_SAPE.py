...
placeholders = {
    'support': [tf.sparse_placeholder(tf.float32) for _ in range(window_size)],# 为稀疏张量插入占位符
    'features':tf.placeholder(tf.float32, shape=(window_size,word_pad_length,feature_dimension)),#占位符 分配内存
    'labels': tf.placeholder(tf.float32, shape=(num_path,)),
    'labels_mask': tf.placeholder(tf.int32,shape=(num_path,word_pad_length)),
    'dropout': tf.placeholder_with_default(0., shape=()),#未输入时 生成默认值
...
with tf.Session() as sess:
  ...
  # Downstream Application 指定代码变量的作用域
  with tf.variable_scope('DownstreamApplication'):
    global_step = tf.Variable(0, trainable=False, name='global_step')
    learn_rate = tf.train.exponential_decay(lr, global_step, FLAGS.decay_step, 0.98, staircase=True)#对学习率learning_rate应用指数衰减
    labels = placeholders['labels']
    Mask = placeholders['labels_mask']
    initializer = tf.keras.initializers.he_normal()##以0为中心的截断正态分布中抽取样本
    print("---\n model.H2 shape: {}\n----".format(model.H2.shape))
    zero_padding = tf.constant(0.0, shape=[1, 8])#生成常量
    # tf.nn.embedding_lookup：选取一个张量里面索引对应的元素 tf.concat：拼接张量，第0维度
    ...
    #tf.tile：同一维度上的复制 tf.expand_dims：增加一个维度
    ...
    print("---\n logits shape: {}\n----".format(logits.shape))
    loss = tf.nn.weighted_cross_entropy_with_logits(targets=labels, logits=logits,pos_weight = 3.) #计算加权交叉熵
    loss = tf.reduce_mean(loss)#计算平均值
    params = tf.trainable_variables()
    optimizer = tf.train.AdamOptimizer(learn_rate)
    grad_and_vars = tf.gradients(loss, params)
    clipped_gradients, _ = tf.clip_by_global_norm(grad_and_vars, 1)#让权重的更新限制在一个合适的范围
    opt = optimizer.apply_gradients(zip(clipped_gradients, params), global_step=global_step) #把计算出来的梯度更新到变量上去
    print("HERE3")
  
  def model_summary():
    model_vars = tf.trainable_variables()#查看可训练的变量
    slim.model_analyzer.analyze_vars(model_vars, print_info=True)#根据指定的tensor获取指定类型的summary操作，主要可用于运行过程中的参数记录
  ...
  idx = np.random.RandomState(seed=42).permutation(len(ally))#打乱索引
  train_word_input = list(map(train_word_input.__getitem__, idx))#重新对应排序
  train_tuopu_input = list(map(train_tuopu_input.__getitem__, idx))
  ally = list(map(ally.__getitem__, idx))
  validation_size = 200#选择验证集
  ...
  if FLAGS.train == True:
    ...
    for epoch_num in range(num_epochs):
      ...
      for i in range(int(total)):
        ...
        batch_input = batch_input.todense()#返回矩阵
        ...
        train_ops = [opt, loss, learn_rate, global_step] #上面的更新进行训练
