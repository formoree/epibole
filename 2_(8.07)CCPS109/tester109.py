...
def one_function(f, test_cases, expected=None, recorder=None, known=None):
...
def all_functions(module, suite, recorder=None, known=None):
    if recorder:
    ...
    for (fname, test_cases, expected) in sort_by_source(suite):
        try:
            ...
            continue
        total += 1
        result = one_function(f, test_cases, expected, recorder, known)
        if result >= 0:
            count += 1
    if recorder:
